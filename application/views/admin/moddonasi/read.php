						...
						<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
							...
							<option value="selected">Export Selected</option>
                                    ...
                                    <tr>
                                      ...
                                      <td><?php echo rupiah($row['target_donasi']) ?>
																			</td>
                          							...
                          							<div class="btn-group">
                          								<?php
                          									if ($row['st_donasi'] == 'Publik') {
                          										?>
                          									<a href="<?php echo site_url('donasi/status/' . $row['id_donasi']) . '/' . 'Blokir'; ?>" title="Ubah status ke Blok">
                          										<button class="btn btn-xs btn-success"><i class="ace-icon fa fa-leaf bigger-120"></i></button></a>
                          								<?php
                          									} elseif ($row['st_donasi'] == 'Blokir') {
                          										?>
                          									<a href="<?php echo site_url('donasi/status/' . $row['id_donasi']) . '/' . 'Publik'; ?>" title="Ubah status ke Publik">
                          										<button class="btn btn-xs btn-warning"><i class="ace-icon fa fa-ban bigger-120"></i></button></a>
                          								<?php
                          									}
                          									?>

                                            <a href="<?php echo site_url('donasi/edit/' . $row['id_donasi']) ?>" title="Ubah">
                            									<button class="btn btn-xs btn-info"><i class="ace-icon fa fa-pencil bigger-120"></i></button></a>
                            								<a href="<?php echo site_url('donasi/delete/' . $row['id_donasi']) ?>" title="Delete" onclick="javascript: return confirm('Yakin Mau dihapus <?php echo $row['judul_donasi']; ?>')">
